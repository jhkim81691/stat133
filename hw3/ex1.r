...
returnNonIdcs <- function(data.vector, idcs) {
	non.idcs.observations <- data.vector[((1:length(data.vector))[-idcs])]
...
subsetDfName <- function(data, non.observations, non.names) {
	data.observations.idcs <- (1:nrow(data))[-non.observations]
	data.variables.idcs <- dimnames(data)[[2]] != non.names
	data.subset <- data[data.observations.idcs, data.variables.idcs]
	return(data.subset)
}
