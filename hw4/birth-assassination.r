...
generateKids <- function(lambda, kappa, parent) {
    ...
    n.kids <- rpois(1, lambda*parent.life)
    ...
    if (n.kids) {
        ...
        # each child's life
        kid.birthdays <- runif(n.kids) * parent.life + parent$birth.date
        life.lengths <- rexp(n.kids, rate=kappa) + parent$assassination.date
		
		kids <- data.frame(parent.id= parent$child.id, child.id=(1:n.kids),
			birth.date= kid.birthdays, assassination.date= life.lengths)
...
nextGeneration <- function(lambda, kappa, parents) {
    ...
    next.gen <- lapply( 1:nrow(parents), function(x)
		generateKids(lambda, kappa, parents[x, ]))
...
bAGen <- function(lambda, kappa, max.gen) {
    ...
    generation.list[[1]] = data.frame(parent.id = 0, child.id = 0,
		birth.date = 0, assassination.date = rexp(1, rate=kappa))
	...
	y <- NULL
	for (x in 1:max.gen) {
		if ( !is.null(generation.list[[x]]) ) {
		generation.list[[x+1]] <- nextGeneration(lambda, kappa, generation.list[[x]])
		}
	}
...
#print(length(unlist(sim.1))/4-1000) #179 children
#print(length(unlist(sim.2))/4-1000) #390 children
#print(length(unlist(sim.3))/4-1000) #45835 children
...
plot(density(gensSurvived(sim.1), bw=0.35), main="B-A Simulation",
	xlim=c(0,11), xlab="generations survived", col="red")
lines(density(gensSurvived(sim.2), bw=0.35), col="green")
lines(density(gensSurvived(sim.3), bw=0.35), col="blue")
legend("topright", legend=c("sim.1", "sim.2", "sim.3"), col=c("red","green","blue"), pch=1)