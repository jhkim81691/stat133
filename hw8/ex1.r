...
compute.bmi = function(heights, weights)  {
    if (length(heights)==length(weights)) return(weights / (heights^2))
...
all$df = data.frame(
	heights = c(female$df$heights, male$df$heights),
	weights = c(female$df$weights, male$df$weights),
	age = c(female$df$age, male$df$age),
	heights_in_in = c(female$df$heights_in_in, male$df$heights_in_in),
	weights_in_lbs = c(female$df$weights_in_lbs, male$df$weights_in_lbs),
	bmi = c(female$df$bmi, male$df$bmi),
	gender = factor(c(rep('f', female$n), rep('m', male$n)))
)
...
colors=c('red', 'blue')
plot(all$df[, 1:3], col=colors[match(all$df$gender, c('f', 'm'))])
...
plot(all$df[, 1:6], col=colors[match(all$df$gender, c('f', 'm'))])
...
par(mfrow=c(2,2))
plot(predict(pca)[, c(1,2)], col=colors[match(all$df$gender, c('f', 'm'))])
plot(predict(pca)[, c(2,3)], col=colors[match(all$df$gender, c('f', 'm'))])
plot(predict(pca)[, c(1,3)], col=colors[match(all$df$gender, c('f', 'm'))])
plot(predict(pca)[, c(3,4)], col=colors[match(all$df$gender, c('f', 'm'))])

# (1 point) Create a kmeans object using all$df
# set k to 2 and make sure that you use 10 different initial conditions

full.km <- kmeans(all$df[, 1:6], 2)

# (1 point) Create a hclust object using all$df

full.hclust = hclust(dist(all$df[, 1:6]))
